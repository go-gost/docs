...
:    Enable HTTP PUT upload support. When enabled, files can be uploaded via PUT requests to the server.
...
If you need to temporarily access the file service through the public network, you can use the public reverse proxy service provided by [Wisper](https://wisper.gost.run) to anonymously expose the local file service to the public network for access.
...
The `006478add9ed096a` in the `endpoint` field is a temporary public access point generated by server and is valid for 24 hours. This file service can be accessed immediately through [https://006478add9ed096a.gost.run](https://006478add9ed096a.gost.run).
