...
`entryPoint`指定流量的入口点，同时通过`ingress`选项指定[Ingress](/concepts/ingress/)对象来定义流量路由规则。
...
在Ingress中可以通过将隧道标记为私有来限制对隧道的访问，由入口点进入的流量无法路由到私有隧道。
...
在Ingress的规则中，通过在endpoint所代表的隧道ID值前添加`$`便将此规则对应的隧道标记为私有，例如上面的srv-2.local主机对应的隧道ac74d9dd-3125-442a-a7c1-f9e49e05faca即为私有隧道，因此通过入口点80端口进入的流量无法使用此隧道。

!!! note "私有性"
    私有性的作用范围为Ingress的规则，而不是隧道本身，同一个隧道在不同的规则中可以有不同的私有性。例如上面例子当中，srv-2.local和srv-3.local使用的是相同的隧道，但srv-3.local对应规则中隧道不是私有的，因此通过入口点80端口进入去往srv-3.local主机的流量可以路由到此隧道。
...
=== "命令行"
    ...
    ```bash
    gost -L tcp://:8000?sniffing=1 -F relay://:8443?tunnelID=ac74d9dd-3125-442a-a7c1-f9e49e05faca
    ```
...
访问端需要在转发器中指定目标节点地址，此地址为虚拟主机名。此名称应当与服务端Ingress中所要匹配的规则所指定的主机名一致(端口号会被忽略)。因此对于TCP流量必须是主机名和隧道ID与Ingress中的规则定义同时匹配上才能正确路由。