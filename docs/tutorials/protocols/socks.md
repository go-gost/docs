...
GOST支持Tor的SOCKS5扩展命令`0xF0` RESOLVE（主机名解析）和`0xF1` RESOLVE_PTR（反向DNS查询），允许`tor-resolve`和`torsocks`等工具通过GOST转发到Tor SOCKS5上游代理。此功能需要通过`tor`选项显式开启。 
...
:    启用Tor SOCKS5扩展命令支持。可通过`tor=true`，`enableTor=true`或`socks5.tor=true`配置。
