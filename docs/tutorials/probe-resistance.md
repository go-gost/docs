...
```
gost -L=http://admin:123456@:8080?probeResistance=code:400&knock=www.example.com
```
...
* `code` - 对应value为HTTP响应码，代理服务器会回复客户端指定的响应码。例如：
    ```
    gost -L=http://admin:123456@:8080?probeResistance=code:403
    ```

* `web` - 对应的value为URL，代理服务器会使用HTTP GET方式访问此URL，并将响应返回给客户端。例如: 
    ```
    gost -L=http://admin:123456@:8080?probeResistance=web:example.com/page.html
    ```

* `host` - 对应的value为主机地址，代理服务器会将客户端请求转发给设置的主机地址，并将主机的响应返回给客户端，代理服务器在这里相当于端口转发服务。例如：
	```
	gost -L=https://admin:123456@:443?probeResistance=host:www.example.com:8080
	```

* `file` - 对应的value为本地文件路径，代理服务器会回复客户端200响应码，并将指定的文件内容作为Body发送给客户端。例如：
	```
	gost -L=http2://admin:123456@:443?probeResistance=file:/send/to/client/file.txt
	```
...
```
gost -L=http://admin:123456@:8080?probeResistance=code:403&knock=www.example.com
```