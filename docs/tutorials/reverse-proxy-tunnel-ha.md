...
在上一篇[反向代理隧道](reverse-proxy-tunnel.md)教程中，详细的讲述了反向代理隧道的功能和使用方法。在本篇教程中将侧重于服务的部署及系统的高可用性方面。
...
服务端无法做到水平扩展的原因是，服务端实例之间是相互独立的，彼此无法感知到其他实例中隧道的信息。因此我们需要一种方法来让服务端共享所有的客户端隧道和连接信息。反向代理隧道服务中通过[服务注册和发现](../concepts/sd.md)机制来达到此目的，但其内部并没有集成具体的服务注册和发现功能模块，而是通过插件的方式将功能开放出来，由用户自己选择实现方式。
